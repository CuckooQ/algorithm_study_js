...

{
  ...
  function testToMaxSingCntAndMaxDVDCnt() {
    ...
    const inputSingTimes = Array.from(
      { length: MAX_SING_CNT },
      (_, idx) => idx + 1
    );
    ...
    const condition =
      testFunction(inputDVDCount, inputSingTimes) === expectResult;
  ...
  function testToMaxSingCntAndMinDVDCntAndMaxDVDTimes() {
    ...
    const expectResult = Math.ceil(
      (MAX_DVD_TIMES * MAX_SING_CNT) / MIN_SING_CNT
    );
    const testFunction = solution;
    const condition =
      testFunction(inputDVDCount, inputSingTimes) === expectResult;
