...

{
        ...
        function sortUnitTimelines() {
            unitTimelines.sort((prev, post) => {
                if(prev[UNIT_TIME_IDX] === post[UNIT_TIME_IDX]) {
                    return prev[UNIT_KIND_IDX].charCodeAt() - post[UNIT_KIND_IDX].charCodeAt(); 
                } else {
    ...
    */
    ...
    function testToMinParticipantCnt() {
        const testNum = 2;
        const input = Array.from({length: MIN_PARTICIPANT_CNT}, (_, idx) => [idx % 72, (idx % 72) + 1]);   
    ...
    function testToAllSameTimeline() {
        const testNum = 3;
        const input = Array.from({length: 10}, () => [MIN_TIMELINE, MAX_TIMELINE]);
    ...
    function main() {
        const input = [[14, 18], [12, 15], [15, 20], [20, 30], [5, 14]];
