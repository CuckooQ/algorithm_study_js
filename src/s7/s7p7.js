 ...
 */

 {
    ...
    function testToMaxCoordinateCnt() {
        const testNum = 1;
        const input = Array.from({length: MAX_COORDINATE_CNT}, (_, idx) => [idx+1, idx+1]).reverse();
    ...
    function testToMinCoordinateCnt() {
        const testNum = 2;
        const input = Array.from({length: MIN_COORDINATE_CNT}, (_, idx) => [idx+1, idx+1]).reverse();
        const expectResult = input.reverse().join("\n");
        const testFunction = solution;
        const condition = (testFunction(input) === expectResult);    
        validateTestResult(testNum, condition);
    ...
    function main() {
        const input = [[2, 7], [1, 3], [1, 2], [2, 5], [3, 6]];
