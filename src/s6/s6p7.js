...

{
  ...
  function testToMaxLenRequiredClassAndMaxLenPlannedClass() {
    const testNum = 1;
    const inputRequiredClasses = Array.from(
      { length: MAX_CLASS_LEN },
      (_, idx) => String.fromCharCode(65 + idx)
    ).join("");
    const inputPlannedClasses = Array.from(
      { length: MAX_CLASS_LEN },
      (_, idx) => String.fromCharCode(65 + idx)
    ).join("");
    ...
    const condition =
      testFunction(inputRequiredClasses, inputPlannedClasses) === expectResult;
