...
// *0일 때의 처리에서 간과한 것이 있었다. 주의하자.
 ...
 {  
    ...
    function isExistPuppet(board, idx) {
        const selectedBoardLine = Array.from(board[idx]);
        selectedBoardLine.reverse();
        return selectedBoardLine.find(puppet => puppet !== 0) ? true : false;
    }

    function selectPuppet(board, idx) {
        while (board[idx].length !== 0) {
            const selectedPuppet = board[idx].pop();
            if (selectedPuppet === 0) {
                continue;
            } else {
    ...
    function main() {
        const inputBoard = [[0,0,0,0,0], [0,0,1,0,3], [0,2,5,0,1], [4,2,4,4,2], [3,5,1,3,1]];
