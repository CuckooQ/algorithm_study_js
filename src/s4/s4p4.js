...

{
  ...
  function getGivablePresentCounts(budget, studentInfoList) {
    ...
    for (let i = 0; i < studentInfoList.length; i++) {
      ...
      let count = 0;
      remainedMoney -=
        studentInfoList[i][PRODUCT_IDX] * SALE +
        studentInfoList[i][DELIVERY_IDX];
      if (remainedMoney <= 0) {
        continue;
      } else {
        count++;
      ...
      for (let j = 0; j < studentInfoList.length; j++) {
        if (i !== j) {
          remainedMoney -=
            studentInfoList[j][PRODUCT_IDX] + studentInfoList[j][DELIVERY_IDX];
          if (remainedMoney < 0) {
            break;
          } else {
            count++;
  ...
  function sortStudentInfoListAsc(studentInfoList) {
    return studentInfoList.sort(
      (befInfo, aftInfo) => befInfo[0] + befInfo[1] - (aftInfo[0] + aftInfo[1])
    );
  }

  function getMaxGivablePresentCount(budget, studentInfoList) {
    const sortedStudentInfoList = sortStudentInfoListAsc(studentInfoList);
    const givablePresentCounts = getGivablePresentCounts(
      budget,
      sortedStudentInfoList
    );
  ...
  */
  ...
  function testToMaxStudentMaxBudget() {
    ...
    const condition =
      testFunction(inputBudget, inputStudentInfo) === expectResult;
