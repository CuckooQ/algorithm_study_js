/**
 * Title: 최솟값 만들기
 * Content: 길이가 같은 배열 A, B 두개가 있습니다. 각 배열은 자연수로 이루어져 있습니다.
 *          배열 A, B에서 각각 한 개의 숫자를 뽑아 두 수를 곱합니다. 이러한 과정을 배열의 길이만큼 반복하며, 두 수를 곱한 값을 누적하여 더합니다.
 *          이때 최종적으로 누적된 값이 최소가 되도록 만드는 것이 목표입니다.
 *          (단, 각 배열에서 k번째 숫자를 뽑았다면 다음에 k번째 숫자는 다시 뽑을 수 없습니다.)
 *          예를 들어 A = [1, 4, 2] , B = [5, 4, 4] 라면
 *          * A에서 첫번째 숫자인 1, B에서 첫번째 숫자인 5를 뽑아 곱하여 더합니다. (누적된 값 : 0 + 5(1x5) = 5)
 *          * A에서 두번째 숫자인 4, B에서 세번째 숫자인 4를 뽑아 곱하여 더합니다. (누적된 값 : 5 + 16(4x4) = 21)
 *          * A에서 세번째 숫자인 2, B에서 두번째 숫자인 4를 뽑아 곱하여 더합니다. (누적된 값 : 21 + 8(2x4) = 29)
 *          즉, 이 경우가 최소가 되므로 29를 return 합니다.
 *          배열 A, B가 주어질 때 최종적으로 누적된 최솟값을 return 하는 solution 함수를 완성해 주세요.
 * Input Condition: 배열 A, B의 크기 : 1,000 이하의 자연수
 *                  배열 A, B의 원소의 크기 : 1,000 이하의 자연수
 * Output Condition: None
 * Input Example: A = [1,4,2]
 *                B = [5,4,4]
 * Output Example: 29
 */
// Expected: 13:50 - 14:30
// Actual: 13:50 - 14:10
// *복습을 위해 다시 풀기

{
  // 다른 사람 풀이 참고
  function solution(a, b) {
    let answer = 0;

    const len = Math.min(a.length, b.length);
    a.sort((c, d) => c - d);
    b.sort((c, d) => d - c);
    for (let i = 0; i < len; i++) {
      answer += a[i] * b[i];
    }

    return answer;
  }

  /*
  function solution(a, b) {
    let answer = 0;
    const len = a.length;
    for (let i = 0; i < len; i++) {
      const max = Math.max(...a);
      const min = Math.min(...b);
      answer += max * min;
      a.splice(a.indexOf(max), 1);
      b.splice(b.indexOf(min), 1);
    }
    return answer;
  }
  */

  /*
  // DFS 시간 초과
  function solution(A, B) {
    let answer = Number.MAX_SAFE_INTEGER;
    dfs();
    return answer;

    function dfs(sel = [], a = [...A], b = [...B]) {
      if (sel.length === A.length) {
        const sum = sel.reduce((acc, val) => acc + val, 0);
        if (sum < answer) {
          answer = sum;
        }
      }

      for (let i = 0; i < a.length; i++) {
        for (let j = 0; j < b.length; j++) {
          const val = a[i] * b[j];
          const newA = [...a];
          newA.splice(i, 1);
          const newB = [...b];
          newB.splice(j, 1);
          dfs([...sel, val], newA, newB);
        }
      }
    }
  }
  */

  function testToExample1() {
    const testNum = 1;
    const inputA = [1, 2];
    const inputB = [3, 4];
    const expectResult = 10;
    const testFunction = solution;
    const condition = testFunction(inputA, inputB) === expectResult;
    validateTestResult(testNum, condition);
  }

  function testToExample2() {
    const testNum = 2;
    const inputA = [1, 2, 3];
    const inputB = [4, 5, 6];
    const expectResult = 28;
    const testFunction = solution;
    const condition = testFunction(inputA, inputB) === expectResult;
    validateTestResult(testNum, condition);
  }

  function main() {
    console.log("S11P126\n");

    const inputA = [1, 4, 2];
    const inputB = [5, 4, 4];
    const output = this.solution(inputA, inputB);

    // test();
    console.log(`Input: ${inputA.join(" ")}\n${inputB.join(" ")}`);
    console.log(`Output: ${output}\n`);
  }

  function test() {
    testToExample1();
    testToExample2();
  }

  main();
}
