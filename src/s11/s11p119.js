/**
 * Title: [3차] 파일명 정렬
 * Content: 세 차례의 코딩 테스트와 두 차례의 면접이라는 기나긴 블라인드 공채를 무사히 통과해 카카오에 입사한 무지는 파일 저장소 서버 관리를 맡게 되었다.
 ...
 * Input Condition: 입력으로 배열 files가 주어진다.
 ...
 * Output Condition: 위 기준에 따라 정렬된 배열을 출력한다.
 * Input Example: ["img12.png", "img10.png", "img02.png", "img1.png", "IMG01.GIF", "img2.JPG"]
 * Output Example: ["img1.png", "IMG01.GIF", "img02.png", "img2.JPG", "img10.png", "img12.png"]
 */
// Expected: 09:50 - 10:30
// Actual: 09:50 - 10:39

{
  function solution(files) {
    ...
    files.forEach((file) => {
      ...
      const number = file.match(regexp)[0];
      const numberFirstIdx = file.indexOf(number);
      const head = file.slice(0, file.indexOf(number));
      const tailFirstIdx = numberFirstIdx + number.length;
      const tail = file.slice(tailFirstIdx);
      slicedFiles.push([head, number, tail]);
    });

    slicedFiles.sort((a, b) => {
      const [aHead, aNumberStr] = a;
      const [bHead, bNumberStr] = b;
      const aHeadLowerCase = aHead.toLowerCase();
      const bHeadLowerCase = bHead.toLowerCase();

      if (aHeadLowerCase === bHeadLowerCase) {
        const aNumber = Number(aNumberStr);
        const bNumber = Number(bNumberStr);
        if (aNumber === bNumberStr) {
          return;
        }
        return aNumber - bNumber;
      }
      return aHeadLowerCase < bHeadLowerCase ? -1 : 1;
    });

    answer = slicedFiles.map(([head, number, tail]) => {
