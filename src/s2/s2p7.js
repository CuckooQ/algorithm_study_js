...
function testToMaxRowColumn () {
    ...
    const MAXLENGTH = 50;
    const input = [];
    const evenRow = Array.from({length:MAXLENGTH}, (_, i) => (i%2 === 0)? 10:0);   
    const oddRow = Array.from({length:MAXLENGTH}, (_, i) => (i%2 !== 0)? 10:0);
...
function testToMaxValue () {
    ...
    const MAXLENGTH = 50;
    const input = [];
    const evenRow = Array.from({length:MAXLENGTH}, (_, i) => (i%2 === 0)? 100:0);   
    const oddRow = Array.from({length:MAXLENGTH}, (_, i) => (i%2 !== 0)? 100:0);
    for(let i=0; i<MAXLENGTH; i++) {
        if (i%2 === 0){
            input.push(evenRow);
        } else {
