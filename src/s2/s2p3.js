...
class RockScissorPaperUser {
    ...
    isDraw (opponent) {
        return this.selection === opponent.selection;
    }

    isWin (opponent) {
        if (this.selection === this.ROCKSCISSORPAPER.SCISSOR && opponent.selection === this.ROCKSCISSORPAPER.PAPER) {
            return true;
        } else if (this.selection === this.ROCKSCISSORPAPER.ROCK && opponent.selection === this.ROCKSCISSORPAPER.SCISSOR) {
            return true;
        } else if(this.selection === this.ROCKSCISSORPAPER.PAPER && opponent.selection === this.ROCKSCISSORPAPER.ROCK){
            return true;
        } else {
            return false;
        }
...
function testToAwin () {
    ...
    let inputBarr = [1, 1];   
    let expectResult = "A\nA";
    let condition = (testFunction(inputCount, inputAarr, inputBarr) === expectResult);    
...
function testToMaxGame () {
    ...
    const testFunction = solution;
    const condition = (testFunction(inputCount, inputAarr, inputBarr) === expectResult);    
    validateTestResult(testNum, condition);
}
